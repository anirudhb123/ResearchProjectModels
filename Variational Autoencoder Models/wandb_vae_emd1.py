...
##            augmented_sample = sample + np.random.normal(loc=0, scale=5, size=sample.shape)
...
input_cluster = Input(shape=(24, 24, 1), name='input_cluster')
...
z_mean = Dense(32, name='z_mean')(x)  # (48,)
z_log_var = Dense(32, name='z_log_var')(x)  # (48,)
...
x = Dense(288, activation="relu")(x)  # (288,)
x = Dense(6 * 6 * 64, activation="relu")(x)  # (576,)
x = Reshape((6, 6, 64))(x)  # (3, 3, 64)
...
layers = Model(inputs=input_cluster, outputs=[z_mean,z_log_var], name='encoder')
...
print(nomClusterTestLabels.shape)
...
def custom_loss1(y_true, y_pred):
    out = y_pred
    out = tf.cast(out, dtype=tf.float64)
    y_true = tf.cast(y_true, dtype=tf.float64)

    if tf.reduce_mean(emd_model([out, y_true])) < 0:
        reconstruction_loss = fpr_weight * tf.sqrt(tf.reduce_mean(tf.square(emd_model([out, y_true]))))
    else:
        reconstruction_loss = tf.sqrt(tf.reduce_mean(tf.square(emd_model([out, y_true]))))

    z_mean = layers(y_true)[0]
    z_log_var = layers(y_true)[1]
...
modelCallbacks = [EarlyStopping(min_delta=0.001,patience=25),
                    ModelCheckpoint(filepath="/abvol/Pred2.hdf5", save_weights_only=True,
                                save_best_only=True)]
...
model.load_weights("/abvol/Pred2.hdf5")

wandb.finish()

# layers.save("/abvol/emd_models/"+modelName+" (latent space),model")
# model.save("/abvol/emd_models/"+modelName+",model")
...
##def weighted_loss(y_true, y_pred,loss_array):
##    # y_true = y_true[:,:,:,0]
##    # y_pred = y_pred[:,:,:,0]
##
##    emds.append(abs(emd_model([y_true,y_pred])))
##    mses.append(np.mean(np.square(y_true - y_pred)))
##
##    return abs(emd_model([y_true,y_pred]))
##
##
##def weighted_loss1(y_true, y_pred,loss_array):
##    y_true = y_true[:,:,:,0]
##    y_pred = y_pred[:,:,:,0]
##
...
##    return abs(emd_model([y_true,y_pred]))
##
##def weighted_loss2(y_true, y_pred,loss_array):
##    y_true = y_true[:,:,:,0]
##    y_pred = y_pred[:,:,:,0]
##
...
def weighted_loss(y_true, y_pred):
    ...
    if np.mean((emd_model([y_true,y_pred])), axis=(0,1)) < 0:
        return np.sqrt(np.mean(np.square(emd_model([y_true,y_pred])), axis=(1))) * r_factor * fpr_weight
    else:
        return np.sqrt(np.mean(np.square(emd_model([y_true,y_pred])), axis=(1))) * r_factor


def weighted_loss2(z_mean,z_log_var):
...
preds = layers.predict(nomClusterTestData)
...
layers_to_avg = layers.predict(nomClusterTestData)
layers_zvvhbb_to_avg = layers.predict(zvvhbb)
layers_ztautau_to_avg = layers.predict(ztautau)
layers_vbfhh_to_avg = layers.predict(vbfhh)
layers_ttbar_to_avg = layers.predict(ttbar)
...
for i in range(num):
    ...
    layers_to_avg = layers_to_avg +layers.predict(nomClusterTestData)
    layers_zvvhbb_to_avg = layers_zvvhbb_to_avg + layers.predict(zvvhbb)
    layers_ztautau_to_avg = layers_ztautau_to_avg + layers.predict(ztautau)
    layers_vbfhh_to_avg = layers_vbfhh_to_avg + layers.predict(vbfhh)
    layers_ttbar_to_avg = layers_ttbar_to_avg + layers.predict(ttbar)
...
layer_mod = [i/nums for i in layers_to_avg]
layers_zvvhbb = [i/nums for i in layers_zvvhbb_to_avg]
layers_ztautau = [i/nums for i in layers_ztautau_to_avg]
layers_vbfhh = [i/nums for i in layers_vbfhh_to_avg]
layers_ttbar = [i/nums for i in layers_ttbar_to_avg]

print(np.array(preds).shape)
print(np.array(preds_to_avg).shape)
print(nomClusterTestData.shape)


print(layers_zvvhbb.shape)

errors_nomTestData = weighted_loss(nomClusterTestData,tf.convert_to_tensor(preds)) + weighted_loss2(layer_mod[0],layer_mod[1])
errors_sig_zvvhbb = weighted_loss(zvvhbb,tf.convert_to_tensor(preds_zvvhbb)) + weighted_loss2(layers_zvvhbb[0],layers_zvvhbb[1])
errors_sig_ztautau = weighted_loss(ztautau,tf.convert_to_tensor(preds_ztautau)) + weighted_loss2(layers_ztautau[0],layers_ztautau[1])

errors_sig_vbfhh = weighted_loss(vbfhh,tf.convert_to_tensor(preds_vbfhh)) + weighted_loss2(layers_vbfhh[0],layers_vbfhh[1])
errors_sig_ttbar = weighted_loss(ttbar,tf.convert_to_tensor(preds_ttbar)) + weighted_loss2(layers_ttbar[0],layers_ttbar[1])
...
# np.save('/abvol/fpr/fpr (zvv,vae_emd).npy',fpr12)
# np.save('/abvol/tpr/tpr (zvv,vae_emd).npy',tpr12)

# np.save('/abvol/fpr/fpr (ztt,vae_emd).npy',fpr32)
# np.save('/abvol/tpr/tpr (ztt,vae_emd).npy',tpr32)
...
plt.legend()
plt.savefig('/abvol/ROC_Curves/ROC Curve (vae_emd).png')
...
print(str(len(indices_zvvhbb)) + " / " + str(len(zvvhbb_met)))

indices_zvvhbb_2 = np.where(scaled2_zvvhbb > threshold2)[0]

print(str(len(indices_zvvhbb_2)) + " / " + str(len(zvvhbb_met)))

zvvhbb_met = zvvhbb_met[:,0]

# valid_mets = zvvhbb_met[indices_zvvhbb]
...
print(str(len(indices_ztautau)) + " / " + str(len(ztautau_met)))

indices_ztautau_2 = np.where(scaled2_ztautau > threshold4)[0]

print(str(len(indices_ztautau_2)) + " / " + str(len(ztautau_met)))
...
print(str(len(indices_vbfhh)) + " / " + str(len(vbfhh_met)))

indices_vbfhh_2 = np.where(scaled2_vbfhh > threshold6)[0]

print(str(len(indices_vbfhh_2)) + " / " + str(len(vbfhh_met)))
...
print(str(len(indices_ttbar)) + " / " + str(len(ttbar_met)))

indices_ttbar_2 = np.where(scaled2_ttbar > threshold8)[0]

print(str(len(indices_ttbar_2)) + " / " + str(len(ttbar_met)))


mets = np.append(zvvhbb_met,ztautau_met)
valid_mets = np.append(zvvhbb_met[indices_zvvhbb], ztautau_met[indices_ztautau])
...
plt.ylabel('frequency')
plt.legend()
plt.savefig('/abvol/eff_plots/met_efficiency.png')
...
plt.xlabel('met')
plt.ylabel('ratio')
plt.savefig('/abvol/eff_plots/met_efficiency2.png')
...
plt.ylabel('frequency')
plt.legend()
plt.savefig('/abvol/eff_plots/pT_distribution.png')
...
plt.ylabel('frequency')
plt.legend()
plt.savefig('/abvol/eff_plots/pt_efficiency.png')
...
plt.ylabel('ratio')
plt.legend()
plt.savefig('/abvol/eff_plots/pt_efficiency2.png')


print('finished Alohalhololololololholaa')
...
plt.figure(figsize=(8,6))
plt.hist(mu,bins=200,histtype='step',density=True, log=True)
plt.xlabel('mu')
plt.xlim(-0.05,0.05)
plt.ylabel('frequency')
plt.title('mu distribution')
plt.legend()
plt.savefig('/abvol/latent_plots/mu_emd.png')

##plt.figure(figsize=(8,6))
##plt.hist(mu,bins=200,histtype='step',density=True, log=True)
##plt.xlabel('mu (zvvhbb)')
##plt.xlim(-0.05,0.05)
##plt.ylabel('frequency')
##plt.title('mu distribution')
##plt.legend()
##plt.savefig('/abvol/latent_plots/mu_zvvhbb_emd.png')
##
##plt.figure(figsize=(8,6))
##plt.hist(mu,bins=200,histtype='step',density=True, log=True)
##plt.xlabel('mu (ztautau)')
##plt.xlim(-0.05,0.05)
##plt.ylabel('frequency')
##plt.title('mu distribution')
##plt.legend()
##plt.savefig('/abvol/latent_plots/mu_ztautau_emd.png')


#-------------------------------------Sigma Distribution PLOTS---------------------------------------------------
plt.figure(figsize=(8,6))
plt.hist(sigma,bins=200,histtype='step',density=True, log=True)
plt.xlabel('sigma')
plt.xlim(-0.3,0.3)
plt.ylabel('frequency')
plt.title('sigma distribution')
plt.legend()
plt.savefig('/abvol/latent_plots/sigma_emd.png')
...
##plt.title('mu distribution')
##plt.legend()
##plt.savefig('/abvol/latent_plots/sigma_zvvhbb_emd.png')
##
##plt.figure(figsize=(8,6))
##plt.hist(sigma,bins=200,histtype='step',density=True, log=True)
##plt.xlabel('sigma (ztautau)')
##plt.xlim(-0.3,0.3)
##plt.ylabel('frequency')
##plt.title('mu distribution')
##plt.legend()
##plt.savefig('/abvol/latent_plots/sigma_ztautau_emd.png')
