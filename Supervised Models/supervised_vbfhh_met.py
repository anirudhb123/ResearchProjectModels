...
vbfhhTrainingDataLength = int(len(vbfhh)*0.7)
vbfhhValidationDataLength = int(len(vbfhh)*0.1)
vbfhhTestDataLength = int(len(vbfhh)*0.2)
...
trainingData = np.append(nom_met[0:nomTrainingDataLength], vbfhh_met[0:vbfhhTrainingDataLength], axis=0)

validationData = np.append(nom_met[nomTrainingDataLength:nomTrainingDataLength + nomValidationDataLength], vbfhh_met[vbfhhTrainingDataLength:vbfhhTrainingDataLength + vbfhhValidationDataLength], axis=0)

testData = np.append(nom_met[nomTrainingDataLength + nomValidationDataLength:nomTrainingDataLength + nomValidationDataLength + nomTestDataLength], vbfhh_met[vbfhhTrainingDataLength + vbfhhValidationDataLength:vbfhhTrainingDataLength + vbfhhValidationDataLength + vbfhhTestDataLength], axis=0)
...
trainingLabels = np.append(np.zeros(nomTrainingDataLength), np.ones(vbfhhTrainingDataLength))
validationLabels = np.append(np.zeros(nomValidationDataLength), np.ones(vbfhhValidationDataLength))
testLabels = np.append(np.zeros(nomTestDataLength), np.ones(vbfhhTestDataLength))
...
train_weights = np.power(np.reciprocal(np.concatenate((nom_met[0:nomTrainingDataLength], vbfhh_met[0:vbfhhTrainingDataLength]), axis=0)), 3) * 1000
val_weights = np.power(np.reciprocal(np.concatenate((nom_met[nomTrainingDataLength:nomValidationDataLength], vbfhh_met[vbfhhTrainingDataLength:vbfhhValidationDataLength]), axis=0)), 3) * 1000
...
train_weights = replace_values(np.concatenate((nom_met[0:nomTrainingDataLength], vbfhh_met[0:vbfhhTrainingDataLength]), axis=0), 100)
val_weights = replace_values(np.concatenate((nom_met[nomTrainingDataLength:nomValidationDataLength], vbfhh_met[vbfhhTrainingDataLength:vbfhhValidationDataLength]), axis=0), 100)



modelCallbacks = [EarlyStopping(patience=25),
                  ModelCheckpoint(filepath="/abvol/Pred_vbfhh.hdf5", save_weights_only=True,
                                  save_best_only=True)]
...
model.save("/abvol/supervised_models/vbfhh" + modelName + ",model")
...
np.save('/abvol/fpr/fpr (vbfhh,supervised - met).npy', fpr)
np.save('/abvol/tpr/tpr (vbfhh,supervised - met).npy', tpr)

plt.figure(figsize=(8,6))

plt.plot(fpr, tpr, label='vbfhh, AUC = ' + str(auc))
...
plt.legend()
plt.savefig('/abvol/ROC_Curves/ROC Curve (vbfhh,supervised - met).png')
...
mets = np.concatenate([nom_met[(-1 * nomTestDataLength):], vbfhh_met[(-1 * vbfhhTestDataLength):]])

valid_mets = np.array(mets[indices1])

np.save('/abvol/mets/mets (vbfhh,supervised - met).npy', mets)
np.save('/abvol/val_mets/val_mets (vbfhh,supervised - met).npy', valid_mets)
...
plt.legend()
plt.savefig('/abvol/eff_plots/met_efficiency (vbfhh, supervised - met).png')
...
plt.xlim(0,300)
plt.legend()
plt.savefig('/abvol/eff_plots/met_efficiency (vbfhh, supervised - met), log.png')
...
np.save('/abvol/bins/(vbfhh,supervised - met).npy', bins1)
np.save('/abvol/divisors/(vbfhh,supervised 0 met).npy', divisors)
...
plt.xlim(0,300)
plt.ylim(0,1)
plt.savefig('/abvol/eff_plots/met_efficiency2 (vbfhh, supervised - met).png')
...
plt.xlim(0,300)
plt.ylim(0.01,1)
plt.savefig('/abvol/eff_plots/met_efficiency2 (vbfhh, supervised - met), log.png')
