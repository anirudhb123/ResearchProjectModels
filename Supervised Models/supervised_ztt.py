...
ztautauTrainingDataLength = int(len(ztautau)*0.7)
ztautauValidationDataLength = int(len(ztautau)*0.1)
ztautauTestDataLength = int(len(ztautau)*0.2)
...
trainingData = np.append(nomClusterData[0:nomTrainingDataLength],ztautau[0:ztautauTrainingDataLength],axis=0)

validationData = np.append(nomClusterData[nomTrainingDataLength:nomTrainingDataLength + nomValidationDataLength],ztautau[ztautauTrainingDataLength:ztautauTrainingDataLength + ztautauValidationDataLength],axis=0)

testData = np.append(nomClusterData[nomTrainingDataLength + nomValidationDataLength:nomTrainingDataLength + nomValidationDataLength + nomTestDataLength],ztautau[ztautauTrainingDataLength + ztautauValidationDataLength:ztautauTrainingDataLength + ztautauValidationDataLength + ztautauTestDataLength],axis=0)


trainingLabels = np.append(np.zeros(nomTrainingDataLength),np.ones(ztautauTrainingDataLength))                           
validationLabels = np.append(np.zeros(nomValidationDataLength),np.ones(ztautauValidationDataLength))
testLabels = np.append(np.zeros(nomTestDataLength),np.ones(ztautauTestDataLength))



##trainingData = np.append(nomClusterData[0:ztautauTrainingDataLength],ztautau[0:ztautauTrainingDataLength],axis=0)
##
##validationData = np.append(nomClusterData[ztautauTrainingDataLength:ztautauTrainingDataLength + ztautauValidationDataLength],ztautau[ztautauTrainingDataLength:ztautauTrainingDataLength + ztautauValidationDataLength],axis=0)
##
##testData = np.append(nomClusterData[ztautauTrainingDataLength + ztautauValidationDataLength:ztautauTrainingDataLength + ztautauValidationDataLength + ztautauTestDataLength],ztautau[ztautauTrainingDataLength + ztautauValidationDataLength:ztautauTrainingDataLength + ztautauValidationDataLength + ztautauTestDataLength],axis=0)
##
##
##trainingLabels = np.append(np.zeros(ztautauTrainingDataLength),np.ones(ztautauTrainingDataLength))                           
##validationLabels = np.append(np.zeros(ztautauValidationDataLength),np.ones(ztautauValidationDataLength))
##testLabels = np.append(np.zeros(ztautauTestDataLength),np.ones(ztautauTestDataLength))
...
modelCallbacks = [EarlyStopping(patience=25),
                    ModelCheckpoint(filepath="/abvol/Pred_ztt.hdf5", save_weights_only=True,
                                save_best_only=True)]

nom_met = nom_met[:,0]
ztautau_met = ztautau_met[:,0]

train_weights = np.power(np.reciprocal(np.concatenate((nom_met[0:nomTrainingDataLength], ztautau_met[0:ztautauTrainingDataLength]), axis=0)),3) * 1000
val_weights = np.power(np.reciprocal(np.concatenate((nom_met[nomTrainingDataLength:nomValidationDataLength], ztautau_met[ztautauTrainingDataLength:ztautauValidationDataLength]), axis=0)),3) * 1000
...
model.save("/abvol/supervised_models/ztt"+modelName+",model")
...
np.save('/abvol/fpr/fpr (ztt,supervised).npy', fpr)
np.save('/abvol/tpr/tpr (ztt,supervised).npy', tpr)

plt.figure(figsize=(8,6))

plt.plot(fpr,tpr,label='ztautau, AUC = ' + str(auc))
...
plt.legend()
plt.savefig('/abvol/ROC_Curves/ROC Curve (ztt,supervised).png')
...
mets = np.concatenate([nom_met[(-1 * nomTestDataLength):], ztautau_met[(-1 * ztautauTestDataLength):]])

valid_mets = np.array(mets[indices1])

np.save('/abvol/mets/mets (ztt,supervised).npy', mets)
np.save('/abvol/val_mets/val_mets (ztt,supervised).npy', valid_mets)
...
plt.legend()
plt.savefig('/abvol/eff_plots/met_efficiency (ztt, supervised).png')
...
plt.xlim(0,300)
plt.legend()
plt.savefig('/abvol/eff_plots/met_efficiency (ztt, supervised), log.png')
...
np.save('/abvol/bins/(ztt,supervised).npy', bins1)
np.save('/abvol/divisors/(ztt,supervised).npy', divisors)
...
plt.xlim(0,300)
plt.ylim(0,1)
plt.savefig('/abvol/eff_plots/met_efficiency2 (ztt, supervised).png')
...
plt.xlim(0,300)
plt.ylim(0.01,1)
plt.savefig('/abvol/eff_plots/met_efficiency2 (ztt, supervised), log.png')
